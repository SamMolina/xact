# X-Act Framework — Taxonomy

Hierarchy only (categories → modules → sub-modules → parameters). Maturity scores are intentionally excluded.

| Level | Element |
|-------|---------|
| 1st | Category |
| 2nd | Module |
| 3rd | Sub-module |
| 4th | Parameter |
| 5th | Question *(not contemplated yet)* |

---

## 1. Software Engineering

- **1.1 Architecture Quality**
  - 1.1.1 Architectural Style
    - 1.1.1.1 Components
    - 1.1.1.2 Dependencies
    - 1.1.1.3 Data model
  - 1.1.2 API Strategy
    - 1.1.2.1 Standards
    - 1.1.2.2 API basic hygiene
    - 1.1.2.3 API best practices
  - 1.1.3 Technology Stack
    - 1.1.3.1 Technology Stack
  - 1.1.4 Performance
    - 1.1.4.1 Resilience
    - 1.1.4.2 Scalability
    - 1.1.4.3 Availability
  - 1.1.5 Governance
    - 1.1.5.1 Architecture decisions
    - 1.1.5.2 Conformance
- **1.2 DevOps**
  - 1.2.1 Continuous Integration and Deployment
    - 1.2.1.1 Build
    - 1.2.1.2 Deployment
  - 1.2.2 Production Operations
    - 1.2.2.1 Observability
    - 1.2.2.2 Business Continuity
  - 1.2.3 Environments
    - 1.2.3.1 Provisioning and use
    - 1.2.3.2 Best Practices
- **1.3 Quality Assurance**
  - 1.3.1 Build In Quality Mindset
    - 1.3.1.1 Business Goal Comprehension
    - 1.3.1.2 Project Technical Comprehension
  - 1.3.2 Test Strategy
    - 1.3.2.1 Test Preparedness
    - 1.3.2.2 Sensible Defaults
  - 1.3.3 Testing Organization Setup
    - 1.3.3.1 Test Process
    - 1.3.3.2 Metrics
    - 1.3.3.3 Test Management
  - 1.3.4 Test Pyramid
    - 1.3.4.1 Unit Testing
    - 1.3.4.2 Functional Testing
    - 1.3.4.3 Automation
- **1.4 Code Quality**
  - 1.4.1 Readability
  - 1.4.2 Maintainability
  - 1.4.3 Re-usability
  - 1.4.4 Evolvability
  - 1.4.5 Complexity
  - 1.4.6 Testability
  - 1.4.7 Tech Debt
  - 1.4.8 Logging
  - 1.4.9 Exception Handling
- **1.5 Agile Project Management**
- **1.6 Engineering Excellence**
  - 1.6.1 Requirement Analysis
    - 1.6.1.1 Requirement Analysis
    - 1.6.1.2 Requirement Design
  - 1.6.2 Architecture and Design
    - 1.6.2.1 Architecture Engagement
    - 1.6.2.2 Documentation
  - 1.6.3 Development
    - 1.6.3.1 Development Discipline
    - 1.6.3.2 Developer Onboarding & Cultivation
    - 1.6.3.3 Source Control Practices
    - 1.6.3.4 Data Management Practices
  - 1.6.4 Deployment
    - 1.6.4.1 Pipeline
    - 1.6.4.2 Cloud Practices
  - 1.6.5 Quality Assurance
    - 1.6.5.1 Shift-Left Testing/Quality
  - 1.6.6 CFRs
    - 1.6.6.1 Accessibility
    - 1.6.6.2 Logging
- **1.7 DevSecOps**
  - 1.7.1 DevSecOps Pipeline
    - 1.7.1.1 Planning & Tracking
    - 1.7.1.2 Configuration Management
    - 1.7.1.3 Solution Development
    - 1.7.1.4 Integration
    - 1.7.1.5 Verification & Validation
    - 1.7.1.6 Deployment
    - 1.7.1.7 Monitor & Control
    - 1.7.1.8 Hosting Services
    - 1.7.1.9 Software Assurance
    - 1.7.1.10 Quality Assurance

## 2. Product and Design

- **2.1 WIP - Project Management**
- **2.2 Product Management**
- **2.3 ProductOps**

## 3. Data Platform

- **3.1 Data readiness**
- **3.2 Data architecture**
- **3.3 Data security and performance**
- **3.4 Data Governance**

## 4. Operational Efficiency

- **4.1 Product & platform stability**
- **4.2 People, process & tools**
- **4.3 Infrastructure Stability**
- **4.4 Optimised Evolvability**
- **4.5 Site Reliability Engineering (SRE)**

## 5. Cloud Platform

- **5.1 WIP - Cloud Modernization**
- **5.2 Cloud Operating Model**
- **5.3 Cloud readiness**
- **5.4 FinOps**
- **5.5 Cloud Architecture**

## 6. Talent Development

- **6.1 Learning Assessment**

## 7. Talent Acquisition

- **7.1 Talent Acquisition**

## 8. Security

- **8.1 Cloud Security Ops**

## 9. Industry 4.0

- **9.1 Manufacturing value chain assessment**
- **9.2 Smart Factory Assessment Index**

## 10. Change Management

- **10.1 Change Strategy and Vision**
- **10.2 Stakeholder Alignment and Engagement**
- **10.3 Communication & Enablement**
- **10.4 Adoption & Behavior Change**
- **10.5 Training & Learning Readiness**
- **10.6 Support & Transition Readiness**
- **10.7 Change Governance & Measurement**

## 11. AI Maturity

- **11.1 AI Strategy & Value Alignment**
- **11.2 Data Readiness & Foundations**
- **11.3 AI Architecture & Platform Enablement**
- **11.4 AI in Product & Customer Experience**
- **11.5 AI in Engineering & Delivery**
- **11.6 AI Governance, Ethics & Risk**
- **11.7 Talent, Skills & Ways of Working**
- **11.8 AI Adoption & Change**

## 12. Org Design

- **12.1 Digital readiness**
- **12.2 Capability and culture**
- **12.3 Regulation readiness**
- **12.4 Vendor optimisation**
